...
let state = {};
let tvboxDeviceModeReady = false;
let tvboxRemoteKeyReady = false;
const TVBOX_FOCUS_SELECTOR = [
  'button:not(:disabled)',
  'input:not(:disabled)',
  'select:not(:disabled)',
  '.tvbox-card',
...
function isLikelyTvDevice() {
  ...
  const wide = Math.min(window.innerWidth || 0, window.innerHeight || 0) >= 540 && Math.max(window.innerWidth || 0, window.innerHeight || 0) >= 960;
  return /smart-tv|smarttv|hbbtv|appletv|googletv|android tv|aftt|aftm|bravia|tizen|webos|hisense|miuibrowser.*tv|box|ott/i.test(ua) || (coarse && wide);
}

function initDeviceMode() {
  if (tvboxDeviceModeReady) return;
  tvboxDeviceModeReady = true;
  const apply = function() {
    const root = document.documentElement;
    const isTv = isLikelyTvDevice();
    root.classList.toggle('tvbox-device-tv', isTv);
    root.classList.toggle('tvbox-device-mobile', !isTv && window.matchMedia && window.matchMedia('(pointer: coarse)').matches);
  };
  apply();
...
function syncFocusable() {
  requestAnimationFrame(function(){
    const root = document.getElementById('tvbox-app');
    if (!root) return;
    root.querySelectorAll(TVBOX_FOCUS_SELECTOR).forEach(function(el){
      if (!el.hasAttribute('tabindex')) el.setAttribute('tabindex', '0');
      el.classList.add('tvbox-focusable');
    });
    const active = document.activeElement;
    if ((!active || active === document.body) && document.documentElement.classList.contains('tvbox-device-tv')) {
      const first = root.querySelector('.tvbox-nav-btn.active, .tvbox-focusable');
...
function getFocusableItems() {
  return Array.from(document.querySelectorAll('#tvbox-app .tvbox-focusable'))
    .filter(function(el){
...
function focusByDirection(direction) {
  ...
  const active = document.activeElement && items.includes(document.activeElement) ? document.activeElement : items[0];
...
function handleRemoteKey(event) {
  ...
  if (keyMap[event.key]) {
    if (focusByDirection(keyMap[event.key])) event.preventDefault();
    return;
  }
  if (event.key === 'Enter' || event.key === ' ') {
    const active = document.activeElement;
    if (active && active.classList && active.classList.contains('tvbox-focusable') && active.click) {
  ...
  if (event.key === 'Escape' || event.key === 'Backspace' || event.key === 'BrowserBack') {
    if (S('showPlayer', false)) { event.preventDefault(); closePlayer(); return; }
    if (S('detailVod', null)) { event.preventDefault(); S('detailVod', null); render(); return; }
    if (S('editingSrc', false) !== false) { event.preventDefault(); window.__tvbox.closeEdit(); }
  }
}

function initRemoteKeys() {
  if (tvboxRemoteKeyReady) return;
  tvboxRemoteKeyReady = true;
