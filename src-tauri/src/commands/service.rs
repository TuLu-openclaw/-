...
fn matches_current_gateway_owner_signature(owner: &GatewayOwnerRecord) -> bool {
    if owner.started_by != "星枢OpenClaw" {
        return false;
    }
...
/// 判断 owner 记录是否可由当前实例使用：必须已经是当前面板写入，且端口/目录/CLI 签名匹配。
/// 注意：运行中的无 owner Gateway 不能在状态轮询里自动认领，否则会把其它安装实例误判为当前实例。
fn should_trust_gateway_owner(owner: &Option<GatewayOwnerRecord>, pid: Option<u32>) -> bool {
    owner
        .as_ref()
...
fn ensure_owned_gateway_or_err(pid: Option<u32>) -> Result<(), String> {
    ...
    if should_trust_gateway_owner(&owner, pid) {
    ...
    fn start_gateway_direct() -> Result<(), String> {
        ...
        cmd.spawn().map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                "OpenClaw CLI 未找到，请确认已安装并重启 星枢OpenClaw。".to_string()
            } else {
...
mod platform {
    ...
    pub(crate) fn cleanup_zombie_gateway_processes() {
        ...
        for line in output.lines() {
            ...
            let pid_str = *parts.last().expect("parts.len() >= 5 guarantees last()");
    ...
    fn create_gateway_log_files() -> Result<(std::fs::File, std::fs::File), String> {
        ...
        let _ = writeln!(
            stdout_log,
            "\n[{}] [星枢OpenClaw] Hidden-start Gateway on Windows",
...
pub async fn get_services_status() -> Result<Vec<ServiceStatus>, String> {
    ...
    for label in labels.iter().map(String::as_str) {
        let (running, pid) = current_gateway_runtime(label).await;
        let owner = read_gateway_owner();
        let owned_by_current_instance = running && should_trust_gateway_owner(&owner, pid);
