...
/// 默认 OpenClaw 配置目录（星枢OpenClaw 自身配置始终在此）
...
/// 获取 OpenClaw 配置目录
/// 优先使用 星枢OpenClaw.json 中的 openclawDir 自定义路径，不存在则回退默认 ~/.openclaw
...
/// 并尊重 `星枢OpenClaw.json` 中的 `openclawDir` 自定义配置目录。
...
fn panel_config_path() -> PathBuf {
    // 星枢OpenClaw 自身配置始终在默认目录，不随 openclawDir 变化
    default_openclaw_dir().join("星枢OpenClaw.json")
}
